...
export default function Home() {
  ...
  // ========== ERROR HANDLING ==========
  if (error && (error.includes("not found") || error.includes("404"))) return <ErrorPage statusCode={404} message={error} />;
  if (error.includes("500")) return <ErrorPage statusCode={500} message={error} />;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className={`flex flex-col min-h-[90vh] transition-all duration-300 ${isDarkMode ? "bg-gray-800 text-white" : "bg-white text-black"}`}>
        <header className={`px-12 py-4 flex justify-between items-center transition-all duration-300 ${isDarkMode ? "bg-black" : "bg-white"} max-sm:px-4`}>
          <h1 className="text-red-500 text-2xl max-sm:text-xl font-bold transition-colors duration-300">
            <i className="fa-brands fa-youtube mr-2"></i> Vidrate
          </h1>
        ...
        {/* =========== FORM & RESULT =========== */}
        <main className="flex flex-col items-center justify-start grow px-4 py-8 overflow-hidden">
          <section className="w-full max-w-3xl flex flex-col items-center">
            <div className="mt-8 flex gap-4 max-md:flex-col max-md:items-center">
              ...
              <button onClick={handleSubmit} className="bg-red-500 text-white border-none text-xl px-4 py-3 w-36 max-md:w-24 max-md:px-2 max-md:py-1 max-md:rounded-md max-[375px]:text-sm cursor-pointer transition-all duration-300 hover:bg-teal-600">Submit</button>
            </div>
      ...
      <footer className={`px-4 py-4 mt-auto h-[10vh] transition-all duration-300 ${isDarkMode ? "bg-black" : "bg-white"}`}>
        <p className={`text-center transition-colors duration-300 ${isDarkMode ? "text-white" : "text-red-500"}`}>
          Made with &#x1FA77; by{" "}
          <a href="https://manishmk.netlify.app" target="_blank" rel="noopener noreferrer" className="text-blue-400 no-underline hover:underline transition-all duration-300">
            Manish Kumar
          </a>{" "}
          , &copy; 2024
        </p>
